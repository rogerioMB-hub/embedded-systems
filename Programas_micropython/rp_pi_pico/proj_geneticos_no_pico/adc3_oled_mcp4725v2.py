...
#criando led para pisca-pisac
led1 = Pin(25, Pin.OUT)

#criando chave-teclado
sw1 = Pin(2, Pin.IN)
...
class individuo(object):
    ...
    def get_volts(self):
        dac.write(self.dec)
        return (ADC1.read_u16() * 3.3 / 65535.0 )
        
    def calc_norm_value(self):
        self.norm_value = self.get_volts() / 3.3
...
while not(sw1.value()):
    ...
    oled.fill(0)
    oled.text("Ajuste tensao:",0,0)
    oled.text("ADC0: ",5,18)
    oled.text(str(round(volts,5)),45,18)
    oled.show()
    time.sleep_ms(50)
norm_v = volts / 3.3
...
pop.show_best()
errop = abs((volts-(i_best.norm_value * 3.3))/volts) * 100
...
print("Erro% = ", errop, "% - ")
print("Erro_min% = ", min_erro, "% - ")
print("Histórico da pop.: ")
print(pop.hist)
print(tempo,"ms")

oled.fill(0)
oled.text("ADC0: ",5,5)
oled.text(str(round(volts,4)),45,5)
oled.text("ADC1: ",5,15)
oled.text(str(round(i_best.norm_value * 3.3,4)),45,15)
oled.text("Tempo:",5,25)
oled.text(str(round(tempo,2)),55,25)
oled.text("ms",110,25)
oled.show()
...
oled.fill(0)
oled.text("ADC1: ",5,0)
oled.text(str(round(i_best.norm_value * 3.3,4)),45,0)
oled.text("Erro: ",5,10)
oled.text(str(round(errop,5)),45,10)
oled.text("%",100,10)
oled.text("b2d : ",5,20)
oled.text(str(i_best.dec)+" / "+str(4095),45,20)
oled.show()
